...
# ---------------------------------------------------------------- Jaipur theme: Hawa Mahal page background
from urllib.parse import quote as _q


def _hawa_facade(W: int = 1600) -> str:
    """Full-width Hawa Mahal facade: stepped pink-sandstone tiers packed with jharokha windows and chhatri domes.
    Drawn as a filled silhouette with window openings, so it reads as a building, not a line icon."""
    cx, base = W / 2, 980
    tiers = [(1500, 150), (1320, 140), (1080, 132), (780, 124), (500, 118), (260, 110)]
    fill, win, line = "#E99BBE", "#15122E", "#E99BBE"
    body, holes, lines = [], [], []
    body.append(f'<rect x="{cx - 790}" y="{base}" width="1580" height="30" rx="4"/>')
    y = base
    for t, (w, h) in enumerate(tiers):
        top, x0 = y - h, cx - w / 2
        body.append(f'<rect x="{x0:.0f}" y="{top:.0f}" width="{w}" height="{h}" rx="6"/>')
        n = max(4, int(w / 46))
        nxt = tiers[t + 1][0] if t + 1 < len(tiers) else 0
        for k in range(n):
            wx = x0 + (k + 0.5) * w / n
            r, wy0, wy1 = 11, top + 34, y - 18
            # jharokha: projecting balcony box with an arched window and a small curved roof
            holes.append(f'<path d="M{wx - r:.0f} {wy1:.0f} V{wy0 + r:.0f} A{r} {r} 0 0 1 {wx + r:.0f} {wy0 + r:.0f} V{wy1:.0f} Z"/>')
            lines.append(f'<path d="M{wx - r - 7:.0f} {wy0 - 2:.0f} Q{wx:.0f} {wy0 - 22:.0f} {wx + r + 7:.0f} {wy0 - 2:.0f}"/>')
            lines.append(f'<path d="M{wx - r - 6:.0f} {wy1 + 6:.0f} H{wx + r + 6:.0f}"/>')
            if abs(wx - cx) > nxt / 2 - 6:                       # chhatri on the exposed roof
                body.append(f'<path d="M{wx - 16:.0f} {top:.0f} A16 20 0 0 1 {wx + 16:.0f} {top:.0f} Z"/>')
                lines.append(f'<path d="M{wx:.0f} {top - 20:.0f} V{top - 32:.0f}"/>')
        y = top
    body.append(f'<path d="M{cx - 60} {y} A60 70 0 0 1 {cx + 60} {y} Z"/>')                 # crown dome
    lines.append(f'<path d="M{cx} {y - 70} V{y - 100}"/>')
    holes.append(f'<path d="M{cx - 40} {base} V{base - 70} A40 40 0 0 1 {cx + 40} {base - 70} V{base} Z"/>')  # gateway
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} 1010" preserveAspectRatio="xMidYMax meet">'
            f'<g fill="{fill}" fill-opacity="0.10">{"".join(body)}</g>'
            f'<g fill="{win}" fill-opacity="0.55">{"".join(holes)}</g>'
            f'<g fill="none" stroke="{line}" stroke-opacity="0.24" stroke-width="2">{"".join(lines)}</g></svg>')


FACADE = "data:image/svg+xml," + _q(_hawa_facade())
...
def _skyline_tile() -> str:
    ...
    extra.append('<path d="M320 40 V18 H372 V40"/><path d="M324 18 A22 18 0 0 1 368 18"/><path d="M336 40 V28 A10 10 0 0 1 356 28 V40"/>')
    ...
    return "data:image/svg+xml," + _q(svg)
...
JAIPUR_CSS = f"""
<style>
/* Pink City at dusk: the Hawa Mahal facade rises across the whole page behind the dashboard */
[data-testid="stAppViewContainer"] {{
  background:
    url("{FACADE}") center bottom / max(1400px, 100vw) auto no-repeat fixed,
    radial-gradient(900px 420px at 50% 108%, rgba(233,155,190,.20), transparent 70%) fixed,
    radial-gradient(1200px 520px at 85% -10%, #2E2160 0%, transparent 60%) fixed,
    linear-gradient(180deg, #15122E 0%, #1B1438 55%, #25163A 100%) fixed;
}}
.hero {{ background: linear-gradient(115deg, rgba(30,26,61,.94) 0%, rgba(30,26,61,.86) 60%, rgba(40,35,79,.80) 100%), url("{JAALI}"); }}
.skyline {{ height: 64px; margin: 2.2rem 0 .4rem; background: url("{SKYLINE}") repeat-x left bottom / 600px 64px;
...
/* pulse scale */
.scale {{ background: rgba(30,26,61,.88); border: 1px solid var(--line); border-radius: 16px; padding: .8rem 1rem .9rem; margin: 0 0 1rem; }}
.scale .ttl {{ font-family: var(--display); font-weight: 700; font-size: 1.02rem; margin-bottom: .55rem; }}
.scale .rail {{ position: relative; display: grid; grid-template-columns: 25fr 25fr 20fr 30fr; height: 14px; border-radius: 999px; overflow: visible; }}
.scale .seg {{ height: 14px; }}
.scale .seg:first-child {{ border-radius: 999px 0 0 999px; }} .scale .seg:last-child {{ border-radius: 0 999px 999px 0; }}
.scale .pin {{ position: absolute; top: -9px; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; }}
.scale .pin b {{ font-size: .7rem; background: var(--ink); border: 1px solid var(--line); border-radius: 6px; padding: 0 .3rem; color: var(--text); white-space: nowrap; }}
.scale .pin i {{ width: 2px; height: 14px; background: var(--text); }}
.scale .keys {{ display: grid; grid-template-columns: 25fr 25fr 20fr 30fr; gap: .6rem; margin-top: 1.4rem; }}
.scale .keys div {{ font-size: .84rem; color: var(--muted); line-height: 1.35; }}
.scale .keys b {{ display: block; font-size: .9rem; }}
@media (max-width: 760px) {{ .scale .keys {{ grid-template-columns: 1fr 1fr; }} }}

/* meter cards: one per reading, value against its usual range */
.meters {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: .8rem; }}
.meter {{ background: var(--panel); border: 1px solid var(--line); border-radius: 16px; padding: .9rem 1rem; }}
.meter.hot {{ border-color: var(--acc); box-shadow: inset 4px 0 0 var(--acc); }}
.meter .hd {{ display:flex; justify-content: space-between; align-items: baseline; gap: .5rem; }}
.meter .nm {{ color: var(--muted); font-size: .92rem; }}
.meter .val {{ font-family: var(--display); font-weight: 800; font-size: 1.9rem; line-height: 1.1; color: var(--acc); }}
.meter .tag {{ font-size: .78rem; font-weight: 700; border: 1px solid currentColor; border-radius: 999px; padding: .05rem .5rem; color: var(--acc); white-space: nowrap; }}
.meter .bar {{ position: relative; height: 10px; margin: .7rem 0 .45rem; background: var(--raised); border-radius: 999px; }}
.meter .usual {{ position: absolute; top: 0; bottom: 0; background: rgba(166,159,199,.35); border-radius: 999px; }}
.meter .typ {{ position: absolute; top: -3px; width: 2px; height: 16px; background: var(--muted); }}
.meter .now {{ position: absolute; top: -5px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%; background: var(--acc); border: 3px solid var(--ink); }}
.meter .cmp {{ font-size: .88rem; color: var(--text); }}
.meter .cmp span {{ color: var(--muted); }}
.meter .mean {{ font-size: .88rem; color: #D8D1EE; margin-top: .35rem; line-height: 1.4; }}
.verdict {{ background: rgba(30,26,61,.9); border: 1px solid var(--line); border-left: 6px solid var(--acc); border-radius: 0; padding: 1rem 1.2rem; margin: .3rem 0 1rem; }}
.verdict h3 {{ margin: 0 0 .3rem; font-size: 1.5rem; }}
.verdict p {{ margin: .1rem 0; font-size: 1.02rem; }}
.verdict .todo {{ color: var(--marigold); font-weight: 700; }}
.legend {{ display:flex; flex-wrap: wrap; gap: .5rem 1.2rem; font-size: .86rem; color: var(--muted); margin: .2rem 0 .6rem; }}
...
def pulse_scale(pins: list[tuple[str, int]], words: dict, meanings: dict) -> str:
    """0-100 pulse scale with what each range means, and a pin for each neighbourhood."""
    segs = "".join(f'<div class="seg" style="background:{STATE[s]}"></div>' for s in ["Critical", "Confirmed", "Watch", "Normal"])
    # group neighbourhoods whose pulse is within 3 points, so every label sits at its true position
    groups: list[list[tuple[str, int]]] = []
    for name, p in sorted(pins, key=lambda t: t[1]):
        if groups and p - groups[-1][-1][1] <= 3:
            groups[-1].append((name, p))
        else:
            groups.append([(name, p)])
    rows_end: list[float] = []                          # stack labels upward when they would collide
    for g in groups:
        x = max(3.0, min(97.0, sum(p for _, p in g) / len(g)))
        names = [n for n, _ in g]
        lab = names[0] if len(names) == 1 else (" & ".join(names) if len(names) == 2 else f"{len(names)} areas")
        lab = f"{lab} {round(sum(p for _, p in g) / len(g))}"
        width = 0.62 * len(lab) + 2.5
        row = next((k for k, end in enumerate(rows_end) if x - width / 2 > end), len(rows_end))
        if row == len(rows_end):
            rows_end.append(0.0)
        rows_end[row] = x + width / 2
        tip = ", ".join(names)
        segs += (f'<div class="pin" style="left:{x:.1f}%; top:{-9 - 20 * row}px" title="{esc(tip)}">'
                 f'<b>{esc(lab)}</b><i style="height:{14 + 20 * row}px"></i></div>')
    lift = 20 * max(0, len(rows_end) - 1)
    segs = segs.replace('<div class="seg"', f'<div class="seg"', 1)
    keys = "".join(f'<div><b style="color:{STATE[s]}">{r}: {esc(words[s])}</b>{esc(meanings[s])}</div>'
                   for s, r in [("Critical", "0–25"), ("Confirmed", "25–50"), ("Watch", "50–70"), ("Normal", "70–100")])
    return (f'<div class="scale"><div class="ttl">How to read the pulse: 100 means everything is as usual for this time of day</div>'
            f'<div class="rail" style="margin-top:{14 + lift}px">{segs}</div><div class="keys">{keys}</div></div>')


def meter_cards(stories: list[dict]) -> str:
    out = ['<div class="meters">']
    for s in stories:
        top = max(s["v"], s["high"], s["typ"]) * 1.2 + 1e-9
        if s["feed"] == "traffic_congestion":
            top = 100.0
        pct = lambda v: f"{100 * max(0.0, min(1.0, v / top)):.1f}%"
        tag = (s["category"] or s["word"]).capitalize() if not s["category"] else s["category"]
        cmp = f'Usually <b>{esc(s["usual"])}</b> at this hour'
        extra = f' <span>({esc(s["multiple"])})</span>' if s["multiple"] else ""
        out.append(
            f'<div class="meter{" hot" if s["unusual"] else ""}" style="--acc:{s["colour"]}">'
            f'<div class="hd"><div class="nm">{s["icon"]} {esc(s["name"])}</div><div class="tag">{esc(tag)}</div></div>'
            f'<div class="val">{esc(s["value"])}</div>'
            f'<div class="bar" title="grey band = usual range for this hour"><div class="usual" style="left:{pct(s["low"])};'
            f'width:calc({pct(s["high"])} - {pct(s["low"])})"></div><div class="typ" style="left:{pct(s["typ"])}"></div>'
            f'<div class="now" style="left:{pct(s["v"])}"></div></div>'
            f'<div class="cmp">{cmp}{extra}. Now <b>{esc(s["phrase"])}</b>.</div>'
            f'<div class="mean">{esc(s["meaning"])}</div></div>')
    out.append("</div>")
    return "".join(out)


def legend(items: list[tuple[str, str]]) -> str:
    return '<div class="legend">' + "".join(f'<span><i style="{sty}"></i>{esc(t)}</span>' for sty, t in items) + "</div>"
