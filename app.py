...
import cp_recorder
import cp_ui as ui
import cp_explain as ex

# Streamlit reloads app.py on every change but can keep old copies of helper modules in memory
# (seen on Streamlit Cloud after a git push). Reloading them here guarantees app.py and its helpers match.
for _m in (cc, ui, ex, cp_llm, cp_recorder):
    importlib.reload(_m)

st.set_page_config(page_title="CityPulse — Jaipur", page_icon="💗", layout="wide",
                   initial_sidebar_state="expanded")
st.markdown(ui.CSS + ui.JAIPUR_CSS, unsafe_allow_html=True)
...
def live_board():
    ...
    st.markdown(f'<div class="source">Rainfall, temperature and AQI: <b>{ui.esc(META["data_source"])}</b>. '
                ...
                unsafe_allow_html=True)
    st.markdown(ui.pulse_scale([(("Sindhi Camp" if z == "SC" else ZNAME[z].split()[0]), int(r.pulse_v))
                                for z, r in snap.iterrows()], T["states"],
                               {k: v[1] for k, v in ex.STATE_MEANING.items()}), unsafe_allow_html=True)
...
live_board()
NOW, snap = snap_at(ss.t_idx)
tabs = st.tabs(["What it means", "Timeline", "Events", "Why this score", "What if", "Report a problem",
                "Ask CityPulse", "Alerts", "How it works"])

# =============================================================================== 0. what it means
with tabs[0]:
    wz0 = st.selectbox("Neighbourhood", ZONES.zone_id, format_func=ZNAME.get, key="wm_zone",
                       index=list(ZONES.zone_id).index(snap.h.idxmax() if snap.h.max() >= THR["watch"] else my_zone))
    r0 = snap.loc[wz0]
    word, what, todo = ex.STATE_MEANING[r0.st_v]
    stories = [ex.feed_story(f, r0) for f in cc.FEATURES]
    hot = [s_ for s_ in stories if s_["unusual"] and s_["feed"] != outage]
    if hot:
        said = join_names([f'{s_["name"].lower()} {"are" if s_["feed"] == "incidents_30m" else "is"} {s_["phrase"]}'
                           for s_ in hot])
        story = f"At {NOW:%H:%M}, {said}."
        if len(hot) >= 2:
            story += " These rising together may be linked, but that is not a confirmed cause."
    else:
        story = f"At {NOW:%H:%M}, rain, traffic, road incidents and air quality are all normal for this time of day."
    st.markdown(f'<div class="verdict" style="--acc:{ui.STATE[r0.st_v]}"><h3>{ui.esc(ZNAME[wz0])}: {ui.esc(word)}, '
                f'pulse {int(r0.pulse_v)} out of 100</h3><p>{ui.esc(what)}</p><p>{ui.esc(story)}</p>'
                f'<p class="todo">What to do: {ui.esc(todo)}</p></div>', unsafe_allow_html=True)
    st.markdown("#### Each reading, compared with what is usual here at this hour")
    st.markdown(ui.legend([("background:rgba(166,159,199,.45)", "grey band: the usual range for this hour"),
                           ("background:#A69FC7;width:3px", "line: the typical value"),
                           (f"background:{ui.STATE['Normal']};border-radius:50%;width:12px;height:12px", "dot: right now")]),
                unsafe_allow_html=True)
    st.markdown(ui.meter_cards([dict(s_, meaning="This feed is switched off in the outage test.", unusual=False)
                                if s_["feed"] == outage else s_ for s_ in stories]), unsafe_allow_html=True)
    st.markdown("#### What the three detectors think")
    checks = [("Compared with normal", "Is any reading far from what is usual here at this hour?", r0.stat_score),
              ("Unusual combination (Isolation Forest)", "Is this mix of readings one we rarely see together?", r0.iso_score),
              ("Unusual pattern over the last hour (LSTM)", "Has the last hour moved in a way this area normally doesn't?",
               r0.lstm_score)]
    cols = st.columns(3)
    for c_, (nm, q, v) in zip(cols, checks):
        yes = v >= cc.STRONG
        c_.markdown(f'<div class="meter" style="--acc:{ui.STATE["Critical"] if yes else ui.STATE["Normal"]}">'
                    f'<div class="nm">{ui.esc(nm)}</div><div class="val" style="font-size:1.35rem">'
                    f'{"Yes, unusual" if yes else "No, looks normal"}</div><div class="mean">{ui.esc(q)}</div></div>',
                    unsafe_allow_html=True)
    agree = int(r0.strong_v)
    st.caption(f"{agree} of 3 detectors see a problem. CityPulse only raises an alert when at least 2 agree and it lasts "
               f"at least two readings (10 minutes).")
    with st.expander("What do these words and numbers mean?"):
        for term, meaning in ex.GLOSSARY:
...
with tabs[1]:
    ...
    if view == "Live recorder":
        st.markdown(ui.legend([("background:#F2B544;height:3px", "the reading"),
                               ("border-top:2px dotted #A69FC7;height:0", "typical for this hour"),
                               ("background:rgba(166,159,199,.3)", "usual range"),
                               ("background:rgba(255,90,106,.3)", "detected disruption")]), unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.caption("Each row is a neighbourhood and each column a moment in time. Dark green means calm; yellow, orange "
               "and red mean more unusual. The future stays blank until the replay reaches it.")
...
def event_evidence(e, lang="en"):
    ...
    for ev in e["evidence"]:
        f = ev["feed"]
        mult = ex.times_usual(ev["value"], ev["typical"])
        cat = ex.band(f, ev["value"])[0]
        out.append(f"{cc.NAMES[f]} reached {fmt_feed(f, ev['value'])}"
                   + (f" ({cat.lower()})" if cat and f != "traffic_congestion" else "")
                   + f", when it is usually {fmt_feed(f, ev['typical'])} at that hour"
                   + (f": {mult}" if mult else ""))
    ch = e["channels"]
...
with tabs[2]:
    ...
    for e in reversed(evs):
        ...
        st.markdown(
            ...
            f'<div style="margin:.2rem 0 .4rem"><b>What it meant for residents:</b> '
            f'{ui.esc(ex.EVENT_MEANING.get(e["pattern"], "Several readings were unusual at the same time."))}</div>'
...
with tabs[3]:
    ...
    with cA:
        ...
        parts = [("Compared with normal", W["stat"] * r.stat_score * 100, ui.MARIGOLD),
                 ("Unusual combination (Isolation Forest)", W["iso"] * r.iso_score * 100, "#7CB7FF"),
                 ("Unusual pattern (LSTM)", W["lstm"] * r.lstm_score * 100, ui.ROSE)]
        fig = go.Figure()
        ...
        fig = go.Figure(go.Bar(x=[v for _, v in vals], y=[cc.NAMES[f] for f, _ in vals], orientation="h",
                               marker_color=[ui.MUTED if f == outage else ex.level(v)[2] for f, v in vals],
                               text=[f"{fmt_feed(f, r[f])} vs usual {fmt_feed(f, r[f + '_typical'])}: {ex.level(v)[1]}"
                                     for f, v in vals],
                               textposition="auto"))
        fig.add_vline(x=2, line=dict(color=ui.STATE["Watch"], dash="dot"), annotation_text="clearly unusual")
        fig.update_layout(title="How unusual is each reading? (0 = normal, 2 = clearly unusual, 5+ = extreme)",
    ...
    with cB:
        ...
        st.markdown('<div class="panel"><h4>Checks before anyone is alerted</h4>' + "".join(
            ...
            for ok, t in checks) + '<p class="muted" style="margin:.6rem 0 0;font-size:.9rem">One detector alone '
                                   'never raises an alert, which is what keeps false alarms rare.</p></div>',
                    unsafe_allow_html=True)
...
with tabs[8]:
    ...
    cA.caption(f"Alone, each detector either misses events or raises false alarms. Requiring two of three to agree "
               f"caught {ab['hybrid']['event_recall']:.0%} of events with {ab['hybrid']['false_alarms_per_day']:.1f} "
               f"false alarms per day, versus {ab['stat']['false_alarms_per_day']:.1f} for statistics alone.")
